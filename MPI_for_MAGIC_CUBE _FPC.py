'''
The Numerical Experiments of FPC over MAGIC CUBE-III cluster.
Finding  Worker Computation Time and Decoding Time of for FPC with matrix partitioning  parameter m=1.
We have n worker nodes and s=n-p stragglers.
A is a 12000*15000 matrix over the real number field, which is evenly divided into $p$ column blocks.
'''
...
if rank == 0:
    ...
    Encoding_matrix = np.zeros((n, k), dtype=float);
    for j in range(0, n):
        Encoding_matrix[j, :] = (node_points[j]) ** np.array(list(range(k)));
    ...
    for i in range(0, n):
        ...
        for j in range(0, k):
            W2b[i] = W2b[i] + Coding_B[i, k - 1 - j] * W1b[j];
    work_product = {};
    for i in range(0, n):
        Ai = W2a[i];
        Bi = W2b[i];
    ...
    decoded_block = np.matmul(decoding_mat[0, :], BB);
